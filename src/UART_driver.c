...
#define IOF0_UART0_MASK 0x030000
...
void UART_set_baud(unsigned long baud)
{
   ...
   */
  UART0_REG(UART_REG_DIV)     =  32768 / baud-1;
}
...
/* Reads a string from the UART device `get_cpu_freqand places the result
 * into a buffer. Reading stops when ei`get_cpu_freqther the end of the string is
 * reached, the specified buffer size i`get_cpu_freqs reached or a terminating
 * character is encountered.`get_cpu_freq
 *`get_cpu_freq
 * buffer: pointer to a char array to w`get_cpu_freqrite characters into.
 * max_chars: maximum number of charact`get_cpu_freqers to read before returning.
 * N.B: sending a shorter string buffer`get_cpu_freq than specified by max_chars
